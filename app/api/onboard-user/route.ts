...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (role === "business") {
      ...
      const parseRes = await businessOnBoardSchema.safeParseAsync({
        phoneNumber,
        franchiseName,
        storeLocation,
        email,
      ...
      if (parseRes?.success) {
        ...
        const upsertFracnchise = await prisma.franchise.upsert({
          create: { name: parseRes?.data?.franchiseName },
          update: {},
          where: { name: parseRes?.data?.franchiseName },
        });
        if (user) {
          const store = await prisma.store.create({
            data: {
              ownerId: user?.id,
              location: parseRes?.data?.storeLocation,
              phoneNumber: parseRes?.data?.phoneNumber,
              franchiseId: upsertFracnchise?.id,
            },
          });
          if (store) {
          ...
          return new Response("no user with submitted id found", {
            status: 500,
          });
        }
      } else {
      ...
      if (parseRes?.success) {
        const user = await prisma.user.update({
          data: { phoneNumber: parseRes?.data?.phoneNumber },
          where: { email: parseRes?.data?.email },
        });
        if (user) {
          return Response.json({ status: "success" });
